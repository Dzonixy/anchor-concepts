...
describe("anchor-concepts", () => {
  ...
  const program = anchor.workspace
    .AnchorConcepts as anchor.Program<AnchorConcepts>;
  ...
  it("Is initialized second!", async () => {
    ...
    const initialize_tx = await program.rpc.initialize(proposals[1], {
      accounts: {
        payer: payer.publicKey,
      },
      signers: [payer],
  ...
  it("Is initialized third!", async () => {
    ...
    const initialize_tx = await program.rpc.initialize(proposals.get("third"), {
      accounts: {
        payer: payer.publicKey,
      },
      signers: [payer],
  ...
  it("Is initialized fourth!", async () => {
    const airdrop_tx = await program.provider.connection.requestAirdrop(
      payer.publicKey,
      2000000000
    );

    const proposalEnum = new Proposals();

    const initialize_tx = await program.rpc.initialize(proposalEnum.fourth, {
      accounts: {
        payer: payer.publicKey,
      },
      signers: [payer],
